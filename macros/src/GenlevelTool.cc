...
bool GenlevelTool::Process(Event & event){
  ...
  sort_by_pt<GenParticle>(*event.gencontent->genparticles_hard);
  sort_by_pt<GenParticle>(*event.gencontent->genparticles_final);
  sort_by_pt<GenParticle>(*event.gencontent->genparticles_visibletaus);
  sort_by_pt<GenJet>(*event.gencontent->genjets);
  ...
  if(event.gencontent->genparticles_visibletaus->size() < 1) return false;
  if(event.gencontent->genparticles_visibletaus->at(0).pt() < 50) return false;
  if(event.gencontent->genjets->size() < 1) return false;
  if(event.gencontent->genjets->at(0).pt() < 50) return false;
  ...
  double stmet = event.gencontent->met_from_invis->pt();
