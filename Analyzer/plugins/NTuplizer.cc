...
private:
  ...
  virtual void NtuplizeMuons(edm::Handle<std::vector<pat::Muon>> input_muons, std::vector<Muon>& output_muons, edm::Handle<std::vector<pat::Jet>> reference_jets);
  virtual void NtuplizeElectrons(edm::Handle<std::vector<pat::Electron>> input_electrons, std::vector<Electron>& output_electrons, edm::Handle<std::vector<pat::Jet>> reference_jets, double rho);
  virtual void NtuplizeTaus(edm::Handle<std::vector<pat::Tau>> input_taus, std::vector<Tau>& output_taus, edm::Handle<std::vector<pat::Jet>> reference_jets, edm::Handle<std::vector<reco::GenParticle>> genparticles_pruned,std::vector<reco::GenParticle> reco_genvistaus);
...
void NTuplizer::NtuplizeMuons(edm::Handle<std::vector<pat::Muon>> input_muons, std::vector<Muon>& output_muons, edm::Handle<std::vector<pat::Jet>> reference_jets) {
  for(size_t i=0; i<input_muons->size(); i++){
    pat::Muon patmu = input_muons->at(i);
    Muon m;
    ...
    int jetidx = -1;
    for (unsigned int j=0; j<reference_jets->size(); j++) { // from here: https://github.com/cms-sw/cmssw/blob/6d2f66057131baacc2fcbdd203588c41c885b42c/PhysicsTools/NanoAOD/plugins/LeptonJetVarProducer.cc#L108#L121
      pat::Jet jet = reference_jets->at(j);
...
void NTuplizer::NtuplizeElectrons(edm::Handle<std::vector<pat::Electron>> input_electrons, std::vector<Electron>& output_electrons, edm::Handle<std::vector<pat::Jet>> reference_jets, double rho) {
  ...
  // loop over electrons
  for(size_t i=0; i<input_electrons->size(); i++){
    pat::Electron patele = input_electrons->at(i);
    Electron e;

    int jetidx = -1;
    for (size_t j=0; j<reference_jets->size(); j++) { // from here: https://github.com/cms-sw/cmssw/blob/6d2f66057131baacc2fcbdd203588c41c885b42c/PhysicsTools/NanoAOD/plugins/LeptonJetVarProducer.cc#L108#L121
      pat::Jet jet = reference_jets->at(j);
...
void NTuplizer::NtuplizeTaus(edm::Handle<std::vector<pat::Tau>> input_taus, std::vector<Tau>& output_taus, edm::Handle<std::vector<pat::Jet>> reference_jets,edm::Handle<std::vector<reco::GenParticle>> genparticles_pruned,std::vector<reco::GenParticle> reco_genvistaus) {

  for(size_t i=0; i<input_taus->size(); i++){
    pat::Tau pattau = input_taus->at(i);
    Tau t;
