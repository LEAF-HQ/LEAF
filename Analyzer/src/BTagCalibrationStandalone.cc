...
BTagEntry::BTagEntry(const std::string &csvLine)
{
  ...
  // make parameters
  // string op_string = vec[0];
  // convert L, M, T, shape to 0, 1, 2, 3
  unsigned op = StringToOP(vec[0]);
  ...
  if(!(jf==JetFlavor::FLAV_B || jf==JetFlavor::FLAV_C || jf==JetFlavor::FLAV_UDSG)){
    std::cerr << "ERROR in BTagCalibration: Invalid csv line; JetFlavor not defined: " << csvLine << std::endl;
    throw std::exception();
  }
...
BTagCalibration::BTagCalibration(const std::string &filename){
  std::ifstream ifs(filename);
  if(!ifs){
    std::cerr << "ERROR in BTagCalibration: File does not exist: " << filename;
    throw std::exception();
  }
  readCSV(ifs);
  ifs.close();
...
const std::vector<BTagEntry>& BTagCalibration::getEntries(const BTagEntry::Parameters &par) const {
  ...
  if (!data_.count(tok)) {
    std::cerr << "ERROR in BTagCalibration: (OperatingPoint, measurementType, sysType) not available: " << tok;
    throw std::exception();
  }
  return data_.at(tok);
}
...
private:
  ...
  std::string sysType_;
  std::map<BTagEntry::JetFlavor, std::vector<TmpEntry> > tmpData_;  // first index: jetFlavor
  std::map<BTagEntry::JetFlavor, bool> useAbsEta_;                  // first index: jetFlavor
  std::map<std::string, std::shared_ptr<BTagCalibrationReaderImpl>> otherSysTypeReaders_;
};


BTagCalibrationReader::BTagCalibrationReaderImpl::BTagCalibrationReaderImpl(BTagEntry::OperatingPoint op, const std::string & sysType, const std::vector<std::string> & otherSysTypes): op_(op), sysType_(sysType){
  tmpData_ = { {BTagEntry::JetFlavor::FLAV_B, {}}, {BTagEntry::JetFlavor::FLAV_C, {}}, {BTagEntry::JetFlavor::FLAV_UDSG, {}} };
  useAbsEta_ = { {BTagEntry::JetFlavor::FLAV_B, true}, {BTagEntry::JetFlavor::FLAV_C, true}, {BTagEntry::JetFlavor::FLAV_UDSG, true} };
...
void BTagCalibrationReader::BTagCalibrationReaderImpl::load( const BTagCalibration & c, BTagEntry::JetFlavor jf, std::string measurementType){
  if (tmpData_.at(jf).size()) {
    std::cerr << "ERROR in BTagCalibrationReader: Data for this jet-flavor is already loaded: " << jf << std::endl;
    throw std::exception();
  }
...
double BTagCalibrationReader::BTagCalibrationReaderImpl::eval(BTagEntry::JetFlavor jf, float eta, float pt, float discr) const{
  bool use_discr = (op_ == BTagEntry::OP_RESHAPING);
  if (useAbsEta_.at(jf) && eta < 0) {
    eta = -eta;
  }
...
double BTagCalibrationReader::BTagCalibrationReaderImpl::eval_auto_bounds(const std::string & sys, BTagEntry::JetFlavor jf, float eta, float pt, float discr) const{
...
BTagCalibrationReader::BTagCalibrationReader(BTagEntry::OperatingPoint op, const std::string & sysType, const std::vector<std::string> & otherSysTypes): pimpl( new BTagCalibrationReaderImpl(op, sysType, otherSysTypes)) {}

BTagCalibrationReader::~BTagCalibrationReader() = default;

void BTagCalibrationReader::load(const BTagCalibration & c, BTagEntry::JetFlavor jf, std::string measurementType){
...
double BTagCalibrationReader::eval_auto_bounds(const std::string & sys, BTagEntry::JetFlavor jf, float eta, float pt, float discr) const{
