...
#include "LEAF/Analyzer/include/NeuralNetworkBase.hpp"

// adapted from https://github.com/UHH2/UHH2/blob/RunII_106X_v2/common/src/NeuralNetworkBase.cxx

using namespace std;


NeuralNetworkBase::NeuralNetworkBase(const Config & cfg, const std::string & ModelName, const std::string& ConfigName){
...
void NeuralNetworkBase::CreateInputs(RecoEvent & event){
    NNInputs.clear();
    NNoutputs.clear();

    if (NNInputs.size()!=LayerInputs.size()) throw logic_error("NeuralNetworkBase.cc: Create a number of inputs diffetent wrt. LayerInputs.size()="+to_string(LayerInputs.size()));
}

bool NeuralNetworkBase::process(RecoEvent & event){
    CreateInputs(event);
