...
public:
  ...
  bool    has(const std::string s) const {auto it = m_additionalvariables.find(s); return (it != m_additionalvariables.end());};
  std::string  get(const std::string s) const {if(Config::has(s)){return m_additionalvariables.at(s);} else{throw std::runtime_error("Trying to get non-existent variable '" + s + "' from Config.");}};
  ...
  std::string  analysis_tool() const {return m_analysis_tool;};
  ...
  std::vector<TString> dataset_infilenames() const {return m_datasets[m_idx].infilenames;};
  ...
  std::shared_ptr<TChain> event_chain;
  std::vector<std::shared_ptr<TChain>> m_additional_event_chains = {}; // used for loading additional collections
  int nevt;
  TTree* outputtree;
