...
from DNNutils import SavePandas, SaveNumpy, LoadPandas


class PreprocessInputsBase():
    def __init__(self, inputdir='', outdir='', maxfiles_per_sample=None, runonfraction=1.0, train_test_val_split=(0.8, 0.1, 0.1), colname_category='category', colname_weights='event_weight', dtypes='float32'):
    ...
    def GetInputs(self,format='csv', filter_regex=None):
        inputs = []
        for samplename in self.DefineSamples():
            ...
            for i, inputfile in enumerate(list_of_inputfiles):
                ...
                df = LoadPandas(os.path.join(self.inputdir, inputfile))
                if self.dtypes!='float32':
                    weights = df.filter(regex=self.colname_weights)
                    labels = df.filter(regex=self.colname_category)
                    df.drop(columns=[self.colname_weights, self.colname_category], inplace=True)
                    df = df.astype(self.dtypes)
                    df = pd.concat([df, weights, labels], axis=1, join='inner')
                if filter_regex:
                    df = self.FilterInputs(df,filter_regex)
                inputs.append(df)
    ...
    def Transform(self):
        for scaler in self.scalers.values():
            for mode in ['train', 'val', 'test']:
                scaled_features  = scaler.transform(self.inputs[mode]).astype(self.dtypes)
    ...
    def SaveBase(self, format='csv'):
        ...
        for mode in ['train', 'val', 'test']:
            SavePandas(self.inputs[mode].astype(self.dtypes),  os.path.join(outdir, 'inputs_%s_%s.%s'  %(mode,frac,format)))
            SavePandas(self.weights[mode].astype(self.dtypes), os.path.join(outdir, 'weights_%s_%s.%s' %(mode,frac,format)))
